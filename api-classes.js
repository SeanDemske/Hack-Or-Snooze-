...
class StoryList {
  ...
  async toggleStoryToFavorites(user, storyId) {

    // Check if story is already favorited
    if (user.favorites.some(function(story) {
      return story.storyId === storyId;
    })) {                                             // Story is already favorited, delete
      const response = await axios({
        method: "DELETE",
    ...
    } else {                                          // Story is not stored as a favorite, add
      const response = await axios({
        method: "POST",
        url: `${BASE_URL}/users/${user.username}/favorites/${storyId}`,
        data: {
      ...
      console.log("added to my favorites!");
    }

    // call the API
    const response = await axios.get(`${BASE_URL}/users/${user.username}`, {
      params: {
        token: user.loginToken
      }
    });
  ...
  async deleteStory(user, storyId) {
    ...
    const userResponse = await axios.get(`${BASE_URL}/users/${user.username}`, {
      params: {
        token: user.loginToken
      }
    });
